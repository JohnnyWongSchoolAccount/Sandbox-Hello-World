...
void mousePressedTimeline() { //drawTextMusic(0,0,":", "Time - ");//text subProgram
  if (mouseX >= 0 && mouseX <= 0 + appWidth-(pauseHeight) && mouseY >= 0 && mouseY <= 0 + pauseWidth)
  // Calculate the new position in the song based on where the user clicked
  songs[currentSong].cue((mouseX) * songs[currentSong].length()/appWidth);
  songs[currentSong].play();
}
